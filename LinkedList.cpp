...
// Updated by Kelly Camacho
// added invert(), invertRest( X ), rotate( X )
...
void LinkedList<T>::invert() {
   ...
   bool could_invert = getLength() > 1;
   if (could_invert) {
      //disconnect first from rest
      Node<T> *first_ptr = getNodeAt(0); //point to original first
      Node<T> *curr_node = getNodeAt(1); //point to original second
      invertRest(curr_node); //invert the rest

      //reconnect rest to first in the appropriate direction
      first_ptr->setNext(nullptr);
   }  //end if
...
void LinkedList<T>::rotate(int k) {
   ...
   if(k != 0) {
      Node<T> *end_node = getNodeAt(item_count_ - 1);
      Node<T> *new_end_node = getNodeAt(item_count_ - 2);

      end_node->setNext(head_ptr_); //set next to old first node
      new_end_node->setNext(nullptr); //make new end and disconnect from old end
      head_ptr_ = end_node; //set head ptr to new first
...
void LinkedList<T>::invertRest(Node<T>* current_first_ptr) {
    ...
    if (current_first_ptr != nullptr) {
      Node<T> *next_ptr = current_first_ptr->getNext(); //get next node

      current_first_ptr->setNext(head_ptr_); //point to previous node
      head_ptr_ = current_first_ptr; //head point to curr
      invertRest(next_ptr); //recall 
    } //end if
